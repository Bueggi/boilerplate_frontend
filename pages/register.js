...
const register = () => {
  // protect register page from logged in user
  ...
  // handlesubmit sends request to server
  const handleSubmit = async () => {
    // Password and email validation
    if (inputs.password !== inputs.confirm) {
      toast.error("Deine Passwörter stimmen nicht überein");
      return;
    }

    if (inputs.password.length < 6) {
      toast.error("Dein Passwort muss länger als 6 Zeichen sein");
      return;
    }
